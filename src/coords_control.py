...
goalsx = [ 1.0, 1.0, 1.0, 3.0, 3.5, 6.5, 6.5, 6.0]
goalsy = [ 3.0, 3.5, 4.5, 4.5, 4.5, 4.0, 2.5, 1.0]
...
rospy.loginfo("Arm name= " + group.get_name())
rospy.loginfo("Arm active joints= " + str(group.get_active_joints()))
rospy.loginfo("Arm current joint values= " + str(group.get_current_joint_values()))


gripper = moveit_commander.MoveGroupCommander('gripper')
rospy.loginfo("Gripper name= " + gripper.get_name())
rospy.loginfo("Gripper active joints= " + str(gripper.get_active_joints()))
rospy.loginfo("Gripper current joint values= " + str(gripper.get_current_joint_values()))


display_trajectory_publisher = rospy.Publisher('/move_group/display_planned_path', moveit_msgs.msg.DisplayTrajectory, queue_size=10)

group_variable_values = group.get_current_joint_values()
gripper_variable_values = gripper.get_current_joint_values()
...
def setPlan(nPlan):
	...
	if(nPlan == 0):
		...
		gripper_variable_values[0] = 0.005
		gripper_variable_values[1] = 0.005

		group.set_joint_value_target(group_variable_values)
		gripper.set_joint_value_target(gripper_variable_values)
		
		plan_home = group.plan()
		gripper_home = gripper.plan()
		#group.go(wait=True)
		group.execute(plan_home,wait=True)
		gripper.execute(gripper_home,wait=True)		
		rospy.sleep(5)
		...
		gripper_pick1 = gripper.plan()
		#group.go(wait=True)
		group.execute(plan_pick1,wait=True)
		gripper.execute(gripper_pick1, wait = True)
		rospy.sleep(5)
		#moveit_commander.roscpp_shutdown()
...
while not rospy.is_shutdown():
	...
	if abs(angle_to_goal - t) > 0.2:
		...
		if(abs(inc_x) <= 0.05 and abs(inc_y) <= 0.05):
			...
			if(i == 1 or i == 3 or i == 6): #or i == otro or ....
				f_pick = True
			i = i + 1
			setgoal(i)
			f_goal = True
